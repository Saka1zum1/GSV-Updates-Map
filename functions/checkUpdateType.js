...
exports.handler = async function (event, context) {
    ...
    try {
        ...
        const [rows] = await connection.execute(channel_id ? `
            SELECT
                MAX(CASE WHEN country = ? AND region = ? AND year = ? AND month = ? AND channel_id IN (?, 1408362784424591360, 1240337068715147407) THEN 1 ELSE 0 END) AS has_region,
                MAX(CASE WHEN country = ? AND year = ? AND month = ? AND channel_id IN (?, 1408362784424591360, 1240337068715147407) THEN 1 ELSE 0 END) AS has_country,
                MAX(CASE WHEN region = ? AND year = ? AND channel_id IN (?, 1408362784424591360, 1240337068715147407) THEN 1 ELSE 0 END) AS has_year
            FROM update_reports
        ` : `
            SELECT
